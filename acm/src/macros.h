...
#define ReadCreatureMysql()					\
	...
	crit->iq		= atoi(row[C_IQ]);		\
	crit->alignment		= atoi(row[C_ALIGNMENT]);	\
	read_flags(row[C_FLAGS], crit);				\
	crit->me		= atoi(row[C_ME]);		\
	crit->ma		= atoi(row[C_MA]);		\
	crit->ps		= atoi(row[C_PS]);		\
	crit->pp		= atoi(row[C_PP]);		\
	crit->pe                = atoi(row[C_PE]);		\
	crit->pb                = atoi(row[C_PB]);		\
	crit->spd               = atoi(row[C_SPD]);		\
	str_dup(&crit->race,		row[C_RACE]	);	\
	str_dup(&crit->occ,		row[C_OCC]);		
...
#define WriteCreatureMysql()						\
	...
	add_int(buf,"me",		crit->me		);	\
	add_int(buf,"ma",		crit->ma		);	\
	add_int(buf,"ps",		crit->ps		);	\
        add_int(buf,"pp",               crit->pp                );	\
        add_int(buf,"pe",               crit->pe                );	\
        add_int(buf,"pb",               crit->pb                );	\
        add_int(buf,"spd",              crit->spd               );	\
	add_str(buf,"race",		crit->race		);	\
	add_str(buf,"occ",		crit->occ		);
