...
// FIX TODO
	if (!strcasecmp(crit->name,"pip") || !strcasecmp(crit->name,"borlak") || !strcasecmp(crit->name,"athanos"))
		return 1;
//	if (crit->level == LEVEL_IMP)
//		return 1;
...
void insert_player_input(CREATURE *crit, char *type, char *input)
{
	...
	sprintf(query, "INSERT INTO player_input (playerID, name, type, room, input)	\
			VALUES(%li, '%s', '%s', %li, '%s')",
		crit->socket->id, crit->name, type, crit->in_room->vnum, smash_quotes(input));
