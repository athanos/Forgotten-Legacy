...
void trans(void *obj, void *to)
{
	...
	case TYPE_OBJECT:
		...
		if(!to)
		{
			...
			{
				xobj->owner_id = 0;
				mysql_update_object(xobj, xcrit);
			}
			return;
		}
		...
		case TYPE_CREATURE:
			...
			if(IsPlaying(tocrit) || (xcrit && IsPlayer(xcrit)))
				mysql_update_object(xobj, xcrit);
			return;
		case TYPE_OBJECT:
			toobj = (OBJECT*)to;
...
OBJECT *splitcoins(OBJECT *coins, void *to, long amount)
{
	...
	; newcoins; newcoins = newcoins->next_content)
		if(newcoins->objtype == OBJ_COIN && newcoins->values[0] == coins->values[0])
			break;
...
mudlog("find obj: num is %li arg[%s]", num, argument);
...
void mysql_update_object(OBJECT *obj, CREATURE *crit)
{
	OBJECT *xobj;
	char buf[MAX_BUFFER];

	// this is for quitting players... objects get removed/deleted/transed and it fubars their stuff
	if(crit && !IsPlaying(crit))
		return;
