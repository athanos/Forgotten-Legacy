...
CMD(do_input)
{
	...
	if(!ValidString(arguments[0]))
		sprintf(query, "SELECT * FROM player_input WHERE done=0"); 
	else if(!strindex("completed", arguments[0]))
	{
		if(ValidString(arguments[1]))
			sprintf(query, "SELECT * FROM player_input WHERE done=1 ORDER BY ID DESC LIMIT %d",
				atoi(arguments[1]));
		else
			sprintf(query, "SELECT * FROM player_input WHERE done=1 ORDER BY ID DESC");
	}
	else if(ValidString(arguments[1]) && !strindex("vnum", arguments[1]))
	{
		...
		if(hvnum >= 0 && lvnum >= 0)
			sprintf(query, "SELECT * FROM player_input WHERE type='%s' AND room >= %li AND room <= %li AND done=0", 
				arguments[0], lvnum, hvnum);
		else
			sprintf(query, "SELECT * FROM player_input WHERE type='%s' AND room = %li AND done=0", 
				arguments[0], lvnum);
	}
	else if(is_number(arguments[0]))
	{
		if(!ValidString(arguments[1]) || strcasecmp(arguments[1], "done"))
			sprintf(query, "SELECT * FROM player_input WHERE ID >= %d AND done=0", atoi(arguments[0]));
		else
		{
			...
			if(!ValidString(arguments[2]))
			{
				sendcrit(crit, "You must supply a solution to mark this done.");
				return;
			}
			update = 1;
			sprintf(query, "UPDATE player_input SET done=1, solution='%s' WHERE ID=%d", 
				smash_quotes(all_args(arguments,2)), atoi(arguments[0]));
		}
	}
	else
	{
		...
		{
			if(is_number(arguments[1]))
				sprintf(query, "SELECT * FROM player_input WHERE type='%s' AND ID > %d AND done=0",
					arguments[0], atoi(arguments[1]));
			else
				sprintf(query, "SELECT * FROM player_input WHERE type='%s' AND name LIKE '%s' AND done=0",
					arguments[0], arguments[1]);
		}
		else
			sprintf(query, "SELECT * FROM player_input WHERE type='%s' AND done=0",
				arguments[0]);
	}
	...
	while((row = mysql_fetch_row(result)))
	{
		sendcritf(crit, "%s[%-2d] [%-4s] %s [%d] %s", 
			...
			atoi(row[I_ROOM]),
			row[I_INPUT]);
		if(ValidString(row[I_SOLUTION]))
			sendcritf(crit, "%s  Solution: %s",
				update % 2 == 0 ? "&+y" : "&+Y",
				row[I_SOLUTION]);
		sendcrit(crit, ""); // newline
		update++;
	}
	if(!update)
