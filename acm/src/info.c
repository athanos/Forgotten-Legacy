...
CMD(do_debug)
{
mudlog("you typed: %s", all_args(arguments,0));
...
display_timestamp(18,crit->socket->hrs);
	...
	sendcritf(crit, "Forgotten Legacy has been up %li day%s, %li hour%s, %li minute%s and %li second%s.",
                days, days == 1 ? "" : "s",
...
CMD(do_who)
{
	...
	// show imm list first
	sendcrit(crit,"-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-");
	sendcrit(crit,"                 Forgotten Legacy Mud                    ");
	sendcrit(crit,"-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-");
	if (imms > 0)
...
Channel(do_music,	"MUSIC",	0,		"&+c",	CFLAG_CHANMUSIC)
Channel(do_immtalk,	"IMM",		LEVEL_IMM,	"&+C",	CFLAG_CHANIMM)
...
CMD(do_score)
{
	...
	sendcritf(crit,"Intelligence Quotient (IQ):  %li",crit->iq);
	sendcritf(crit,"Mental Endurance (ME)     :  %li",crit->me);
	sendcritf(crit,"Mental Affinity (MA)      :  %li",crit->ma);
	sendcritf(crit,"Physical Strength (PS)    :  %li",crit->ps);
	sendcritf(crit,"Physical Prowess (PP)     :  %li",crit->ps);
	sendcritf(crit,"Physical Endurance (PE)   :  %li",crit->pe);
	sendcritf(crit,"Physical Beauty (PB)      :  %li",crit->pb);
